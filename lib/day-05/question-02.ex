defmodule Day5Question2 do
  ...
  def lowest_seat_id(seat_codes) do
    seat_codes
    |> Enum.min_by(&seat_id/1)
    |> seat_id()
  end

  def my_seat_id(seat_codes) do
    seat_ids = seat_codes
               |> Enum.sort_by(&seat_id/1)
               |> Enum.map(&seat_id/1)

    Range.new(lowest_seat_id(seat_codes), highest_seat_id(seat_codes))
    |> Enum.to_list()
    |> Kernel.--(seat_ids)
    |> hd()
  end
